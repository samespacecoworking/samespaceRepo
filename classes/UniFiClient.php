...
class Client
{
    ...
    // Samespace additions START

    /**
     *  Create Access User
     *  @param string $first_name First Name of the user to be created
     *  @param string $last_bame Last Name of the user to be cretaed
     *  @param string $employee_number The ID generated by Nexudus for this user
     *  @param string @pin_code The Access PIN for the new user - a string of digits. NB UDM configured to require min 6 or 8 digits
     *  @param array $group_ids Array of UIDs which identify the user group(s) the new user should be placed in
     *
     *  @return string JSON object of newly created user
     */
    public function create_access_user($first_name, $last_name, $employee_number, $pin_code, $group_ids)
    {
        $payload = [
        ...
        return $this->fetch_results('/proxy/access/api/v2/user', $payload, false, true);
    }

    /**
     * Get one Access User
     * @param string $name_or_employeeid name or Employee ID of the use to be retrieved
     *
     * @return array returns a JSON array of matching users
     */
    public function get_access_user($name_or_employeeid)
    {
	    return $this->fetch_results('/proxy/access/ulp-go/api/v2/users/search?page_num=1&page-size=10&condition=' . $name_or_employeeid, '', false, true);
    }

    /**
     * Search Access events
     */
    public function get_access_events($from_timestamp, $to_timestamp, $filter)
    {
        $payload = ['filter' => $filter, 'show_avatar' => true, 'since' => $from_timestamp, 'until' => $to_timestamp];
   	return $this->fetch_results('/proxy/access/ulp-go/api/logs/search?page_size=50&page_num=1', $payload, false, true);
    }

    // Samespace additions END
    ...
    protected function fetch_results($path, $payload = null, $boolean = false, $login_required = true)
    {
	    ...
	    /**
	     * to deal with a response from the Access API
	     */
	    if (strpos($path, '/access/') === 0 or strpos($path, '/proxy/access/') === 0) {
		if (isset($response->msg)) {
		    if ($response->msg === 'success' or $response->msg == 'succ') {
	                $this->last_error_message = '';
                        if (!$boolean) {
                            return $response->data;
                        }
                         ...
                         * set $this->set last_error_message if the returned error message is available
                         */
                        $this->last_error_message = $response->error . ' ' . $response.message;
                        if ($this->debug) {
                            trigger_error('Debug: Last error message: ' . $this->last_error_message);
                        }
                    }
    ...
    protected function exec_curl($path, $payload = null)
    {
        ...
        if ($this->is_unifi_os && !(strpos($path, '/proxy/access') === 0) && !(strpos($path, '/access') === 0)) {
